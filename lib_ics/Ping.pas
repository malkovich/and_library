...
//    function    GetOnEchoReply : TICMPReply;
  ...
  public
    ...
    procedure   DnsLookupB(HostName : String); virtual;
...
procedure TPing.DnsLookupB(HostName : String);
var
    IPAddr  : TInAddr;
    HostEnt: PHostEnt;
begin
    ...
    OutputDebugString(PChar('DnsLookup Inet_addr (HostName):' + HostName));

    IPAddr.S_addr := Inet_addr(@HostName[1]);
    if IPAddr.S_addr <> u_long(INADDR_NONE) then
    begin
        OutputDebugString(PChar( HostName + ' ÊÇIPµØÖ·: $' + IntToHex(IPAddr.S_addr,8)));
        FDnsResult := StrPas(inet_ntoa(IPAddr));
        if Assigned(FOnDnsLookupDone) then
            FOnDnsLookupDone(Self, 0);
        Exit;
    ...
    HostEnt := GetHostByName(@HostName[1]);
    if HostEnt <> nil then
    begin   
    ...
    raise Exception.CreateFmt('%s: can''t start DNS lookup, error #%d',
                  [HostName, WSAGetLastError]);
end;
