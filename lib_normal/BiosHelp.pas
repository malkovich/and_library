...
function FindPhoenixBios (var BiosAddr: PChar): Integer;
function FindAmiBios (var BiosAddr: PChar): Integer;
function FindAwardBios (var BiosAddr: PChar): Integer;
...
begin
  ...
  pBiosAddr := Pointer (DWORD(BiosAddr) + $EC71);
  ZeroMemory( @szBiosData[0], 127);
  CopyMemory( @szBiosData[0], pBiosAddr, 127);
  szBiosData[127] := #0;
  iLen := lstrlen (szBiosData);

  if (iLen > 0) and (iLen < 128) then
  begin
    if (szBiosData[2] = '/') and (szBiosData[5] = '/') then
    begin
      P := @szBiosData[0];
      while P^ <> 0 do
      begin
        if (P^ < ord(' ')) or (P^ >= 127) then
          Break;
        Inc (P);
      end;
      if P^ = 0 then
...
begin
  ...
  pBiosAddr := Pointer (DWORD(BiosAddr) + $F478);
  ZeroMemory( @szBiosData[0], 127);
  CopyMemory( @szBiosData[0], pBiosAddr, 127);
  szBiosData[127] := #0;
  iLen := lstrlen (szBiosData);
  ...
  if (iLen > 0) and (iLen < 128) then
  begin
    if (szBiosData[2] = '-') and (szBiosData[7] = '-') then
    begin
      P := @szBiosData[0];
      while P^ <> 0 do
      begin
...
function FindPhoenixBios (var BiosAddr: PChar): Integer;
const
  uOffset: array[0..2] of DWORD = ($6577, $7196, $7550);
var
  pBiosAddr: PChar;
  P: PByte;
  szBiosData: Array[0..127] of char;
...
begin
  ...
  for I := 0 to 2 do
  begin
    pBiosAddr := Pointer (DWORD(BiosAddr) + uOffset[I]);
    ZeroMemory( @szBiosData[0], 127);
    CopyMemory( @szBiosData[0], pBiosAddr, 127);
    szBiosData[127] := #0;
    iLen := lstrlen (szBiosData);
    ...
    if (iLen > 0) and (iLen < 128) then
    begin
      if (szBiosData[7] = '.') and (szBiosData[11] = '.') then
      begin
        P := @szBiosData[0];
        while P^ <> 0 do
        begin
...
//
//
//来自：dangbochang, 时间：2003-6-16 17:47:00, ID：1956626
