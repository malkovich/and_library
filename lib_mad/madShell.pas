// ***************************************************************
//  madShell.pas              version:  1.3o  ? date: 2006-01-05
//  -------------------------------------------------------------
...
type
  ...
  IShortCut = interface (IBasic) ['{00BED963-C78D-11D3-A530-00005A180D69}']
    ...
    property IShellLink_   : IShellLink read GetIShellLink;
    ...
    property  ShortCutFileName    : string read GetShortCutFileName write SetShortCutFileName;
    ...
    function GetFindData : TWin32FindData;
    property FindData    : TWin32FindData read GetFindData;
    ...
    procedure SetHotKey      (value: string );
    ...
    function Save (shortCutFileName : string  = '';
...
type
  ...
  IShellObj = interface (ICustomBasicList) ['{00BED961-C78D-11D3-A530-00005A180D69}']
    ...
    function GetDescription   : string;
    function GetDescrInFolder : string;
    property Description      : string read GetDescription;
    property DescrInFolder    : string read GetDescrInFolder;
    ...
    function NewShortCut (shortCutFileName: string = '') : IShortCut;
    ...
    function Item (name       : string ) : IShellObj; overload;
...
const CErrorNo_CantUndoReadOnly  = CErrorBase_Shell + 0;
      ...
      CErrorStr_CantUndoReadOnly = 'You can''t unprotect an ID list.';
      CErrorStr_ProtectedIDList  = 'This ID list is protected!';
      CErrorStr_NoDesktopItem    = 'This is no desktop item!';

// ***************************************************************

// internal stuff, please ignore
type TWatchType = (wtAdded, wtRemoved, wtModified, wtRenamed);
     TWatchDirCallback   = procedure (watchType: TWatchType; old, new_: wideString);
     TWatchDirCallbackOO = procedure (watchType: TWatchType; old, new_: wideString) of object;
function WatchDir (dir: wideString; watchSubtree: boolean; events: TShellEventTypes;
...
var FWinFolder : string = '';
function WinFolder : string;
begin
  if FWinFolder = '' then begin
    SetLength(FWinFolder, MAX_PATH + 1);
    SetLength(FWinFolder, GetWindowsDirectory(pchar(FWinFolder), MAX_PATH));
  end;
  result := FWinFolder;
end;

var FSysFolder : string = '';
function SysFolder : string;
begin
  if FSysFolder = '' then begin
    SetLength(FSysFolder, MAX_PATH + 1);
    SetLength(FSysFolder, GetSystemDirectory(pchar(FSysFolder), MAX_PATH));
  end;
  result := FSysFolder;
end;
...
constructor TIIDList.Create(valid: boolean; lastErrorNo: cardinal; lastErrorStr: string;
                            pidl: PItemIDList; copy: boolean = false);
var c1 : cardinal;                            
begin
  ...
  if CheckValid then
    if (idList <> nil) and idList.IsValid then begin
      ...
      if pidl1 <> nil then begin
        Move(      FPidl^,        pidl1^,        c1);
        Move(idList.pidl^, (pchar(pidl1) + c1)^, c2);
      end;
...
begin
  if CheckValid then
    if FReadOnly <> value then
      if value then
           FReadOnly := true
      else SetLastError(CErrorNo_CantUndoReadOnly, CErrorStr_CantUndoReadOnly);
end;
...
begin
  ...
  if CheckValid then
    if not FReadOnly then begin
      if FExtractReady then begin
        ...
        if FSize > 2 then begin
          ...
          TPWord(integer(FPidl) + FSize - i2)^ := 0;
          dec(FSize, i1);
          FPNameReady := false;
        end else begin
          FMalloc.Free(FPidl);
    ...
    end else SetLastError(CErrorNo_ProtectedIDList, CErrorStr_ProtectedIDList);
  if result = nil then
    result := TIIDList.Create(false, FLastErrorNo, GetLastErrorStr, nil);
end;
  ...
  if CheckValid then
    if not FReadOnly then begin
      if (idList <> nil) and idList.IsValid then begin
        ...
        Move(idList.pidl^, (pchar(FPidl) + c1)^, c2);
        inc(FSize, c2 - 2);
        FPNameReady   := true;
        FPName        := pointer(cardinal(FPidl) + c1);
        FExtractReady := false;
        FPath         := nil;
        FName         := nil;
      end else SetLastError(ERROR_INVALID_PARAMETER);
...
begin
  ...
  while pidl1^.mkid.cb > 0 do begin
    ...
    cardinal(pidl1) := cardinal(pidl1) + pidl1^.mkid.cb;
  end;
  FSizeReady  := true;
  FPNameReady := true;
end;
...
begin
  ...
  if FSize > 2 then begin
    ...
    TPWord(integer(pidl1) + FSize - i2)^ := 0;
    idl1 := TIIDList.Create(true, 0, '', pidl1);
    idl1.FSize      := FSize - i1;
    idl1.FSizeReady := true;
    idl1.FReadOnly  := true;
    ...
    FPath := TIIDList.Create(false, CErrorNo_AmInvalid, CErrorStr_AmInvalid, nil);
  FExtractReady := true;
end;
...
type
  ...
  public
    ...
    constructor Create (valid: boolean; lastErrorNo: cardinal; lastErrorStr: string;
                        const idList: IIDList; fileName: string);
    destructor Destroy; override;
...
constructor TIShortCut.Create(valid: boolean; lastErrorNo: cardinal; lastErrorStr: string;
                              const idList: IIDList; fileName: string);
var c1    : cardinal;
    pidl1 : PItemIDList;
begin
  ...
  if FValid then begin
    ...
    if FValid then begin
      ...
      c1 := cardinal(CoCreateInstance(CLSID_ShellLink, nil,
                                      ...
                                      IShellLink, FShellLink));
      if c1 = 0 then
        if idList = nil then begin
          c1 := cardinal((FShellLink as IPersistFile).Load(PWideChar(wideString(fileName)), 0));
          if c1 = 0 then begin
...
function TIShortCut.GetIShellLink : IShellLink;
begin
  result := FShellLink;
end;

function TIShortCut.GetShortCutFileName : string;
begin
  result := FFile;
end;

procedure TIShortCut.SetShortCutFileName(value: string);
begin
  if CheckValid then
...
begin
  if FIDList = nil then
    FIDList := TIIDList.Create(false, CErrorNo_AmInvalid, CErrorStr_AmInvalid, nil);
  result := FIDList;
end;

function TIShortCut.GetPath : string;
...
function TIShortCut.GetFindData : TWin32FindData;
...
function TIShortCut.GetDescription : string;
var arrCh : array [0..MAX_PATH] of char;
    c1    : cardinal;
begin
  result := '';
...
function TIShortCut.GetParams : string;
var arrCh : array [0..MAX_PATH] of char;
    c1    : cardinal;
begin
  result := '';
...
function TIShortCut.GetWorkingDir : string;
var arrCh : array [0..MAX_PATH] of char;
    c1    : cardinal;
begin
  result := '';
...
function TIShortCut.GetHotKey : string;
var w1 : word;
    c1 : cardinal;
begin
  result := '';
...
function TIShortCut.GetIconPath : string;
var arrCh : array [0..MAX_PATH] of char;
    i1    : integer;
    c1    : cardinal;
begin
...
procedure TIShortCut.SetPath(value: string);
var c1 : cardinal;
begin
  if CheckValid then begin
    c1 := cardinal(FShellLink.SetPath(pchar(value)));
    if c1 = 0 then FDirty := true
    else           SetLastError(c1);
  end;
end;

procedure TIShortCut.SetDescription(value: string);
var c1 : cardinal;
begin
  if CheckValid then begin
    c1 := cardinal(FShellLink.SetDescription(pchar(value)));
    if c1 = 0 then FDirty := true
    else           SetLastError(c1);
  end;
end;

procedure TIShortCut.SetParams(value: string);
var c1 : cardinal;
begin
  if CheckValid then begin
    c1 := cardinal(FShellLink.SetArguments(pchar(value)));
    if c1 = 0 then FDirty := true
    else           SetLastError(c1);
  end;
end;

procedure TIShortCut.SetWorkingDir(value: string);
var c1 : cardinal;
begin
  if CheckValid then begin
    c1 := cardinal(FShellLink.SetWorkingDirectory(pchar(value)));
    if c1 = 0 then FDirty := true
    else           SetLastError(c1);
  end;
end;

procedure TIShortCut.SetHotKey(value: string);
var w1 : word;
    c1 : cardinal;
begin
  if CheckValid then begin
    w1 := TextToShortCut(value);
...
procedure TIShortCut.SetIconPath(value: string);
var c1 : cardinal;
begin
  if CheckValid then begin
    FIconPath := value;
    if FIconIndex = -1 then begin
      ...
      c1 := cardinal(FShellLink.SetIconLocation(pchar(FIconPath), FIconIndex));
      if c1 <> 0 then begin
        FIconIndex := 0;
        c1 := cardinal(FShellLink.SetIconLocation(pchar(FIconPath), FIconIndex));
      end;
...
function TIShortCut.SetIcon(iconPath: string; iconIndex: integer) : boolean;
var c1 : cardinal;
begin
  result := false;
  if CheckValid then begin
    ...
    c1 := cardinal(FShellLink.SetIconLocation(pchar(iconPath), iconIndex));
    if c1 = 0 then begin
      FDirty := true;
      result := true;
    end else
...
function TIShortCut.Save(shortCutFileName: string = ''; tryToResolve: boolean = true) : boolean;
var s1 : string;
    c1 : cardinal;
begin
  result := false;
    ...
    if (FFile <> '') or (shortCutFileName <> '') then begin
      try
        if FDirty or ((shortCutFileName <> '') and (not IsTextEqual(shortCutFileName, FFile))) then begin
          ...
          s1 := ExtractFileExt(shortCutFileName);
          if IsTextEqual(s1, '.pif') or IsTextEqual(s1, '.lnk') then
            DeleteR(shortCutFileName, 4);
          if (not tryToResolve) or Resolve(INVALID_HANDLE_VALUE, 0) then begin
            c1 := cardinal((FShellLink as IPersistFile).Save(PWideChar(wideString(shortCutFileName + '.lnk')), true));
            if c1 = 0 then result := true
            else           SetLastError(c1);
          end;
...
function NewShortCut(path: string) : IShortCut;
begin
...
type
  ...
  public
    ...
    function  NeedStr               (shgdn : cardinal; out str: string) : boolean;
...
begin
  ...
  if OS.winNt then begin
    ...
    if @SharedMem9x_Alloc = nil then
      SharedMem9x_Alloc := GetProcAddress(LoadLibrary('comctl32.dll'), pchar(71));
    if @SharedMem9x_Alloc <> nil then
...
begin
  if OS.winNt then begin
    ...
    if @SharedMem9x_Free = nil then
      SharedMem9x_Free := GetProcAddress(LoadLibrary('comctl32.dll'), pchar(73));
    if @SharedMem9x_Free <> nil then
      SharedMem9x_Free(buf);
  end;
end;

constructor TIShellObj.Create(valid: boolean; lastErrorNo: cardinal; lastErrorStr: string;
                              const parentShellFolder: IShellFolder;
                              const idList, childIdList: IIDList);
var c1 : cardinal;
begin
...
function TIShellObj.GetDescription : string;
begin
...
function TIShellObj.GetDescrInFolder : string;
begin
...
function TIShellObj.GetName : string;
begin
  if (not FNameReady) and CheckValid then NeedName;
  result := FName;
end;

function TIShellObj.GetPath : string;
begin
  if (not FNameReady) and CheckValid then NeedName;
  result := FPath;
end;

procedure TIShellObj.SetName(value: string);
var c1    : dword;
    pidl1 : PItemIDList;
begin
  if CheckValid and NeedParentShellFolder and NeedSplittedIDLists then begin
    c1 := FParentShellFolder.SetNameOf(0, FChildIDList.pidl, PWideChar(wideString(value)), 0, pidl1);
    if c1 = 0 then
      FMalloc.Free(pidl1)
    else
      SetLastError(c1);
  ...
  case Msg of
    ...
    WM_DRAWITEM, WM_MEASUREITEM, WM_INITMENUPOPUP:
      begin
        ...
        end else cm3.HandleMenuMsg2(msg, wParam, lParam, result);
      end;
    else
      result := DefWindowProc(wnd, msg, wParam, lParam);
  end;
      ...
      try
        ...
        if c1 and $80000000 = 0 then begin
          ...
          if (cm.QueryInterface(IContextMenu2, cm2) = NO_ERROR) or
            ...
            ZeroMemory(@wndCl, sizeOf(TWndClass));
            ...
            wnd := CreateWindow(wndCl.lpszClassName, 'ShellObjContextSubMenuHelper',
...
function TIShellObj.ExecContextItem(parentWnd : cardinal;
  ...
  function FindMenuItem(menu: cardinal; item: string) : cardinal;
  ...
  begin
    ...
    for i2 := 0 to i1 - 1 do begin
      ...
      if GetMenuItemInfo(menu, i2, true, mii) and
...
function TIShellObj.NewShortCut(shortCutFileName: string) : IShortCut;
begin
  result := TIShortCut.Create(FValid, CErrorNo_AmInvalid, CErrorStr_AmInvalid,
...
begin
  if CheckValid then
    if FDesktopItem then begin
      lv := FindWindowEx(FindWindowEx(FindWindow('Progman', 'Program Manager'), 0, 'SHELLDLL_DefView', nil), 0, 'SysListView32', nil);
      if lv <> 0 then begin
          ...
          try
            ...
            if p1 <> nil then
              try
                ...
                buf.findItem.psz   := @p1^.arrCh;
                if WriteProcessMemory(ph, p1, @buf, sizeOf(buf), c1) and (c1 = sizeOf(buf)) then begin
                  i1 := SendMessage(lv, LVM_FINDITEM, -1, integer(@p1^.findItem));
                  if i1 <> -1 then begin
                    if SendMessage(lv, LVM_SETITEMPOSITION, i1, pos.x or (pos.y shl 16)) = 0 then
                      SetLastError(CErrorNo_Unknown, CErrorStr_Unknown);
                  end else self.SetLastError(ERROR_FILE_NOT_FOUND);
                end;
              finally FreeMemEx(ph, p1) end;
          finally CloseHandle(ph) end;
      end;
    end else SetLastError(CErrorNo_NoDesktopItem, CErrorStr_NoDesktopItem);
end;
...
begin
  ...
  if CheckValid and NeedParentShellFolder and NeedSplittedIDLists then begin
    ...
    else i1 := FParentShellFolder.GetUIObjectOf(dword(-1), 1, pidl1, IExtractIconW, nil, pointer(eiA));
    if i1 = 0 then begin
      if askAgain <> nil then
           c1 := GIL_ASYNC
      else c1 := 0;
      ...
      if i2 = 0 then begin
        if OS.winNt then
             i2 := integer(eiW.Extract(arrChW, i1, HICON(c1), HICON(c2), size))
        else i2 := integer(eiA.Extract(arrChA, i1, HICON(c1), HICON(c2), size));
        if i2 = 0 then begin
          if c2 <> 0 then
            DestroyIcon(c2);
          result := c1;
...
begin
  ...
  if (index < 0) or (index >= FCount) then
       result := TIShellObj.Create(false, CErrorNo_IndexOutOfRange, CErrorStr_IndexOutOfRange, nil, nil, nil)
...
var desktopItems  : array of record
  ...
  procedure FindDesktopItem(caption: string; var pos: TPoint; var state: cardinal);
  var i1 : integer;
  begin
              ...
              if ph <> 0 then
                try
                  ...
                  if p1 <> nil then
                    try
                      ZeroMemory(@buf, sizeOf(buf));
                      SetLength(desktopItems, SendMessage(lv, LVM_GETITEMCOUNT, 0, 0));
                      for i1 := 0 to high(desktopItems) do begin
                        ...
                        if WriteProcessMemory(ph, p1, @buf, sizeOf(buf), c1) and (c1 = sizeOf(buf)) and
                           (SendMessage(lv, LVM_GETITEMPOSITION, i1, integer(@p1^.pos)) <> 0) and
                           (SendMessage(lv, LVM_GETITEM, 0, integer(@p1^.lvItem)) <> 0) and
                           ReadProcessMemory(ph, p1, @buf, sizeOf(buf), c1) and (c1 = sizeOf(buf)) then begin
...
function TIShellObj.Item(name: string) : IShellObj;
var i1 : integer;
begin
  if CheckValid then begin
    if not FRefreshReady then RefreshItems;
    if posStrIs1('::{', name) then begin
      if (length(name) >= 40) and (name[40] = '}') then
        try
          result := Item(StringToGuid(Copy(name, 3, 38)));
          exit;
        except end;
    end else begin
      if posStrIs1('{', name) and (length(name) = 38) and (name[38] = '}') then
        try
          result := Item(StringToGuid(name));
...
function TIShellObj.NeedStr(shgdn: cardinal; out str: string) : boolean;
var sr         : TStrRet;
...
begin
  ...
  if shgdn = SHGDN_FORPARSING then begin
    ...
    if SHGetPathFromIDList(FIDList.pidl, pchar(s1)) then begin
      str := pchar(s1);
      result := true;
    end else begin
      NeedShellFolder;
      ...
      repeat
        if (sf1 <> nil) and (sf1.QueryInterface(IPersistFolder, pf) = 0) and
          ...
          str := '::' + GuidToString(guid) + '\' + str;
        end else if (sf2 <> nil) and (sf2.GetDisplayNameOf(idl2.pidl, SHGDN_FORPARSING, sr) = 0) then begin
          case sr.uType of
            STRRET_CSTR   : str := string(sr.cStr)                         + '\' + str;
            STRRET_OFFSET : str := pchar(cardinal(idl2.pidl) + sr.uOffset) + '\' + str;
            STRRET_WSTR   : str := wideString(sr.pOleStr                 ) + '\' + str;
            else            str := '?'                                     + '\' + str;
          end;
        end else break;
        idl2 := idl1.SplitName;
    ...
    if result then begin
      case sr.uType of
        STRRET_CSTR   : str := string(sr.cStr);
        STRRET_OFFSET : str := pchar(cardinal(FChildIDList.pidl) + sr.uOffset);
        STRRET_WSTR   : str := wideString(sr.pOleStr);
        else            str := '?';
      end;
    end else SetLastError(c1 and $FFFF);
  end;
...
function PathToIDList(path: wideString) : IIDList;
    ...
    if path <> '' then begin
      ...
      if c1 <> 0 then begin
        ...
        repeat
          ...
          else if (i1 = 3) and (path[2] = ':') then inc(i1);
...
function ShellObj(path: string) : IShellObj;
begin
  result := TIShellObj.Create(true, 0, '', nil, pathToIDList(path), nil);
end;

function ShellObj(sf: TSpecialFolder) : IShellObj;

  function RegRdStr(key: HKEY; path, name: string; var str: string; var error: cardinal) : boolean;
  var hk     : HKEY;
      c1, c2 : dword;
      s1     : string;
  begin
    result := false;
    error := cardinal(RegOpenKeyEx(key, pchar(path), 0, KEY_QUERY_VALUE, hk));
    if error = 0 then
      try
        SetLength(str, MAX_PATH + 1);
        c1 := MAX_PATH;
        error := cardinal(RegQueryValueEx(hk, pchar(name), nil, @c2, pointer(str), @c1));
        if error = 0 then begin
          str := pchar(str);
          if c2 = REG_EXPAND_SZ then begin
            SetLength(s1, MAX_PATH + 1);
            SetLength(str, c1 - 1);
            if ExpandEnvironmentStrings(pchar(str), pchar(s1), MAX_PATH) > 0 then
              str := pchar(s1);
          end;
          result := true;
        end;
...
//CSIDL_HISTORY                       = $0022;  // ie history folder
  ...
  CRegNames : array [TSpecialFolder] of string = (
    '', '', '', '', '', '', '', '', '', '',
      '', '', '', 'Fonts', '', '', '', '', '', '', '', '',
  ...
  CRegCurrentVersion   = 'Software\Microsoft\Windows\CurrentVersion';
  CRegUserShellFolders = '\Explorer\User Shell Folders';
  CRegShellFolders     = '\Explorer\Shell Folders';
var pidl1 : PItemIDList;
    s1    : string;
    c1    : cardinal;
    idl1  : IIDList;
begin
  ...
  if idl1 = nil then
    if (CRegNames[sf] <> '') and
       ( RegRdStr(HKEY_CURRENT_USER,  CRegCurrentVersion + CRegUserShellFolders, CRegNames[sf], s1, c1) or
         RegRdStr(HKEY_CURRENT_USER,  CRegCurrentVersion + CRegShellFolders,     CRegNames[sf], s1, c1) or
         RegRdStr(HKEY_LOCAL_MACHINE, CRegCurrentVersion + CRegUserShellFolders, CRegNames[sf], s1, c1) or
         RegRdStr(HKEY_LOCAL_MACHINE, CRegCurrentVersion + CRegShellFolders,     CRegNames[sf], s1, c1)    ) then
      idl1 := pathToIDList(s1)
    else
      case sf of
        sfDialupNetwork :
          if OS.win9x then
        ...
        sfWindows :
          idl1 := pathToIDList(winFolder);
        sfSystem :
          idl1 := pathToIDList(sysFolder);
        sfTemp :
          begin
            SetLength(s1, MAX_PATH + 1);
            if GetTempPath(MAX_PATH, pchar(s1)) <> 0 then
                 idl1 := pathToIDList(pchar(s1))
        ...
        sfAllUsersProfile :
          begin
            SetLength(s1, MAX_PATH + 1);
            if ExpandEnvironmentStrings('%ALLUSERSPROFILE%', pchar(s1), MAX_PATH) > 0 then begin
              s1 := pchar(s1);
              if (s1 <> '') and (s1[1] <> '%') then
                   idl1 := pathToIDList(pchar(s1))
...
type
  ...
  TShellEventContainer = class
    ...
    constructor Create (eventProc: TShellEvent; eventProcOO: TShellEventOO;
                        root: string; watchSubtree: boolean; eventTypes: TShellEventTypes);
    destructor Destroy; override;
    procedure ShMsgHandlerProc (window, msg: cardinal; wParam, lParam: integer; var result: integer);
    procedure WatchMsgHandlerProc (window, msg: cardinal; wParam, lParam: integer; var result: integer);
    procedure WatchDirCallback (watchType: TWatchType; old, new_: wideString);
  end;
...
begin
  ...
  if (GetVersion and $80000000 = 0)         and         // NT-Familie
    ...
    FWatchHandle := WatchDir(wideString(root), watchSubtree, eventTypes, nil, WatchDirCallback);
  ...
  if @SHChangeNotifyRegister = nil then begin
    ...
    SHChangeNotifyRegister   := GetProcAddress(dll, pchar(  2));
    SHChangeNotifyDeregister := GetProcAddress(dll, pchar(  4));
    ILFree                   := GetProcAddress(dll, pchar(155));
    SHSimpleIDListFromPath   := GetProcAddress(dll, pchar(162));//157));
  end;
  if eventTypes <> [] then begin
    FShMsg        := AddMsgHandler(ShMsgHandlerProc);
    FLockProc     := GetProcAddress(GetModuleHandle(shell32), pchar(644));
    FUnlockProc   := GetProcAddress(GetModuleHandle(shell32), pchar(645));
    if (root <> '') and (root <> '*') then begin
      if GetVersion and $80000000 = 0 then
           notifyInfo.pidl := SHSimpleIDListFromPath(PWideChar(wideString(root)))
      else notifyInfo.pidl := SHSimpleIDListFromPath(    pchar(           root ));
...
procedure TShellEventContainer.ShMsgHandlerProc(window, msg: cardinal; wParam, lParam: integer; var result: integer);
var pidls  : pointer;
    events : TInternalShellEventTypes;
    lock   : cardinal;

  procedure DoCallback(event: TShellEventType; const obj1, obj2: IShellObj; drive: char; value: cardinal);
  begin
  ...
  var e1           : TInternalShellEventType;
      ...
      i1           : integer;
  begin
    pidl1 := pointer(pointer(cardinal(pidls) + 0)^);
    pidl2 := pointer(pointer(cardinal(pidls) + 4)^);
    for e1 := low(TInternalShellEventType) to high(TInternalShellEventType) do
      if e1 in events then begin
        case e1 of
          ...
          iseImageChanged        : DoCallback(seImageChanged,      nil, nil, #0, cardinal(pointer(cardinal(pidl1) + 2)^));
          ...
          iseFreespaceChanged    : begin
                                     driveMap := cardinal(pointer(cardinal(pidl1) + 2)^);
                                     for i1 := 0 to 25 do
                                       if (1 shl i1) and driveMap <> 0 then
                                         DoCallback(seFreespaceChanged, nil, nil, chr(ord('A') + i1), 0);
                                   end;
          iseExtendedEventPreIe4,
          iseExtendedEvent       : case cardinal(pointer(cardinal(pidl1) + 2)^) of
...
procedure TShellEventContainer.WatchMsgHandlerProc(window, msg: cardinal; wParam, lParam: integer; var result: integer);

  procedure DoCallback(event: TShellEventType; old, new: wideString);
...
procedure TShellEventContainer.WatchDirCallback(watchType: TWatchType; old, new_: wideString);
var infoRec: TWatchDirInfo;
begin
  infoRec.watchType := watchType;
  infoRec.old       := old;
  infoRec.new       := new_;
  SendMessage(MsgHandlerWindow(FThreadID), FWatchMsg, 789, integer(@infoRec));
end;

function NewShellEvent(eventProc: TShellEvent; eventProcOO: TShellEventOO;
                       root: string; watchSubtree: boolean; eventTypes: TShellEventTypes) : boolean;
var i1 : integer;
...
constructor TIDisplayMode.Create(valid: boolean; lastErrorNo: cardinal; lastErrorStr: string;
                                 const devMode: TDevMode);
begin
  inherited Create(valid, lastErrorNo, lastErrorStr);
  if FValid then
...
constructor TIDisplayModes.Create(valid: boolean; lastErrorNo: cardinal; lastErrorStr: string);
...
begin
  if (index < 0) or (index >= FCount) then
       result := TIDisplayMode.Create(false, CErrorNo_IndexOutOfRange, CErrorStr_IndexOutOfRange, dm)
