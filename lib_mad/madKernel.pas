// ***************************************************************
//  madKernel.pas             version:  1.3h  ·  date: 2006-05-21
//  -------------------------------------------------------------
...
// 2006-05-21 1.3h (1) GetHandleTableNt doesn't work in 64bit OSs
...
type
  ...
  IKernelObj = interface (IBasic) ['{72518460-8D63-11D3-A52E-00005A180D69}']
    ...
    function GetObjTypeStr : string;
    property ObjType       : TKernelObjType read GetObjType;
    property ObjTypeStr    : string read GetObjTypeStr;
...
//                           var handled : boolean ) : integer;
  ...
  IWindow = interface (IBasic) ['{F70A8420-C1D6-11D3-A530-00005A180D69}']
    ...
    function ChildWindow (className : string;
    ...
    function  GetProp    (name: string) : cardinal;
    procedure SetProp    (name: string; value: cardinal);
    property  Prop       [name: string] : cardinal read GetProp write SetProp;
    function  DeleteProp (name: string) : boolean;
...
//    function Unhook : boolean;
  ...
  IModule = interface (IBasic) ['{F84BE460-C13F-11D3-A530-00005A180D69}']
    ...
    function GetProcAddress (name  : string ) : pointer; overload;
    function GetProcAddress (index : integer) : pointer; overload;

    // manipulate the address of the exported functions (caution!!)
    function SetProcAddress (name  : string;  newAddr: pointer) : boolean; overload;
...
const CErrorNo_RefreshListError       = CErrorBase_Kernel + 0;
      ...
      CErrorStr_WaitAbandoned         = 'Wait abandoned.';
      CErrorStr_WaitTimeout           = 'Wait timeout.';
      CErrorStr_WaitFailed            = 'Wait failed.';
      CErrorStr_RefreshListError      = 'Can''t refresh this kind of list.';
      CErrorStr_ProcessStartedButNoID = 'Process was started, but Windows returned invalid PID.';
      CErrorStr_NoTopLevelWindow      = 'This function works only on top-level windows.';
      CErrorStr_NoWin32Process        = 'This function works only on win32 processes.';
      CErrorStr_InvalidReplaceArray   = 'Invalid replace array.';
      CErrorStr_CantCopyFunction      = 'This function cannot be copied.';
...
{$ifndef madDisAsm}
  ...
  function Magic : cardinal;
    ...
    begin
      result := not IsBadReadPtr(addr, 4);
      if result then
        value := dword(addr^);
    end;
...
type
  ...
  T9xEDB = packed record
    environment    : pchar;         // GetEnvironmentStrings
    d0             : cardinal;
    cmdLine        : pchar;         // GetCommandLine
    currentDir     : pchar;         // GetCurrentDirectory
...

{
  ...
  TGlobalModule = packed record
    ...
    d2           : dword;
    path         : pchar;             // name (with    path) of the module
    name         : pchar;             // name (without path) of the module
    ...
    d5           : dword;
    path2        : pchar;             // same as path
    path2Len     : word;              // same as pathLen
    name2        : pchar;             // same as name
...
function ModuleInfoNameNt(ph: cardinal; const mi: TNtModuleInfo; var ws: wideString) : boolean;
var c1     : cardinal;
...
begin
  ...
  if (mi.handle <> 0) and (mi.nameLen >= 2) then begin
    ...
    if result then begin
      ws := PWideChar(ws);
      if PosStrIs1('\??\', ws) then begin
        Delete(ws, 1, 4);
      end else if PosTextIs1('\SystemRoot\System32\', ws) then begin
...
begin
  ...
  else if IsTextEqual(ExtractFileName(exeFile), 'winoa386.mod') then result := etDos
  else                                                               result := etUnknown;
end;

function Icon(file_: string; smallIcon, linkOverlayIfLnkOrPif: boolean) : cardinal;
var sfi : TSHFileInfo;
    s1  : string;
    c1  : cardinal;
begin
  ...
  if linkOverlayIfLnkOrPif then begin
    s1 := ExtractFileExt(file_);
    linkOverlayIfLnkOrPif := IsTextEqual(s1, '.lnk') or IsTextEqual(s1, '.pif');
...
type
  ...
  public
    ...
    constructor Create (valid: boolean; lastErrorNo: cardinal; lastErrorStr: string;
                        exeFile: string; const objects: array of IBasic);
...
const
  ...
  CConvertObjTypeNt4 : array [0..$17] of byte =
    ($00, $00, $12, $13, $14, $04, $05, $02, $00, $03, $01, $06, $00, $15, $16, $07, $17, $18, $00, $00, $00, $00, $19, $08);
  CConvertObjType2k  : array [0..$1A] of byte =
    ($00, $00, $12, $13, $14, $04, $05, $00, $02, $00, $03, $00, $01, $06, $00, $15, $16, $07, $17, $18, $07, $00, $00, $00, $00, $19, $08);
  CConvertObjTypeXP  : array [0..$1C] of byte =
    ($00, $00, $12, $13, $14, $04, $05, $00, $00, $02, $00, $03, $00, $01, $06, $00, $1a, $15, $16, $07, $17, $18, $07, $00, $00, $00, $00, $19, $08);
  CObjNames : array [TKernelObjType] of string =
    ('Unknown', 'Semaphore', 'Event', 'Mutex', 'Process', 'Thread', 'Timer', 'FileMapping',
...
begin
  ...
  try
    ...
    if result = nil then begin
      case type_ of
        otEvent   : result := TIEvent         .Create(addr <> nil, CErrorNo_AmInvalid, CErrorStr_AmInvalid, handle);
        otMutex   : result := TIMutex         .Create(addr <> nil, CErrorNo_AmInvalid, CErrorStr_AmInvalid, handle);
        otThread  : result := TIThread        .Create(addr <> nil, CErrorNo_AmInvalid, CErrorStr_AmInvalid, handle);
        otProcess : result := TIProcess       .Create(addr <> nil, CErrorNo_AmInvalid, CErrorStr_AmInvalid, handle);
        else        result := TIOtherKernelObj.Create(addr <> nil, CErrorNo_AmInvalid, CErrorStr_AmInvalid, handle);
      end;
...
constructor TIKernelObj.Create(valid: boolean; lastErrorNo: cardinal; lastErrorStr: string;
...
function TIKernelObj.GetObjTypeStr : string;
var c1  : cardinal;
    buf : TPAPointer;
begin
...
begin
  ...
  try
    ...
    if NtQueryInformationFile(tr^.handle, @iob, @buf, sizeOf(buf), 9) = 0 then begin
      ...
      tr^.str := wideString(buf.str);
    end;
  except sysUtils.ShowException(ExceptObject, ExceptAddr) end;
end;

function TIKernelObj.GetObjName : string;
var c1  : cardinal;
    buf : TPAPointer;
    th  : cardinal;
        ...
        if IsValid then begin
          ...
          if c1 <> 0 then begin
            ...
            try
              ...
              if c1 = 0 then
                   FName := wideString(PWideChar(buf^[1]))
...
begin
  ...
  if FHandle = nil then
    FHandle := TIHandle.Create(false, CErrorNo_AmInvalid, CErrorStr_AmInvalid, 0, 0, false, nil, nil);
  result := FHandle;
end;
...
begin
  ...
  else result := TIHandles.Create(false, CErrorNo_AmInvalid, CErrorStr_AmInvalid, nil, nil, []);
end;

// ***************************************************************

constructor TIHandle.Create(valid: boolean; lastErrorNo: cardinal; lastErrorStr: string;
                            processID, handle: cardinal; autoClose: boolean;
                            hi95: TP9xHandleItem; hiNt: TPNtHandleItem);
var c1  : cardinal;
    ht  : TNtHandleTable;
    ...
    if (handle <> maxCard) and (handle <> 0) then begin
      ...
      if OS.win9x then begin
        ...
        if hiNt <> nil then begin
          FAccess := hiNt^.access;
          FAddr   := hiNt^.objAddr;
          FTypeNo := hiNt^.objType and $FF;
          case OS.winNtEnum of
            osWinNt4 : if FTypeNo > ord(high(CConvertObjTypeNt4)) then
                            FType := otUnknown
                       else FType := TKernelObjType(CConvertObjTypeNt4[FTypeNo]);
            osWin2k  : if FTypeNo > ord(high(CConvertObjType2k)) then
                            FType := otUnknown
                       else FType := TKernelObjType(CConvertObjType2k[FTypeNo]);
            osWinXP, osWin2003 :
                       if FTypeNo > ord(high(CConvertObjTypeXP)) then
                            FType := otUnknown
                       else FType := TKernelObjType(CConvertObjTypeXP[FTypeNo]);
            else       FType := otUnknown;
          end;
        end else begin
          // GetHandleTableNt doesn't work in 64bit XP/w2k3...   :-(
          if @NtQueryObject = nil then
            NtQueryObject := GetProcAddress(GetModuleHandle(ntdll), 'NtQueryObject');
          c1 := 0;
          ...
          if c1 > 0 then begin
            ...
            try
              if NtQueryObject(FHandle, 2, buf, c1, nil) = 0 then
                s1 := wideString(PWideChar(buf^[1]));
            finally FreeMem(buf) end;
          end;
          if s1 <> '' then begin
            FType := otUnknown;
            for ot := low(TKernelObjType) to high(TKernelObjType) do
              if IsTextEqual(CObjNames[ot], s1) then begin
                FType := ot;
                break;
              end;
            if NtQueryObject(FHandle, 0, @obi, sizeOf(obi), nil) = 0 then
              FAccess := obi[1];
...
begin
  if FValid then
    with TIHandle.Create(true, 0, '', FProcess, FHandle, false, nil, nil) do begin
...
begin
  ...
  else result := TIProcess.Create(false, CErrorNo_AmInvalid, CErrorStr_AmInvalid, otProcess, 0);
end;
...
begin
  if FKernelObj = nil then begin
    if CheckValid then begin
      ...
      result := TIOtherKernelObj.Create(false, CErrorNo_AmInvalid, CErrorStr_AmInvalid, otUnknown, 0);
      FKernelObj := result;
    end;
  ...
  if CheckValid then
    if FProcess = GetCurrentProcessID then begin
      case WaitForSingleObject(FHandle, 0) of
        WAIT_TIMEOUT   : begin
                           result := true;
                           SetLastError(WAIT_TIMEOUT, CErrorStr_WaitTimeout);
                         end;
        WAIT_ABANDONED : SetLastError(WAIT_ABANDONED, CErrorStr_WaitAbandoned);
        WAIT_OBJECT_0  : begin
                           case FType of
  ...
  if CheckValid then
    if FProcess = GetCurrentProcessID then begin
      case madKernel.WaitFor([IHandle(self)], false, milliseconds, handleMessages) of
        WAIT_TIMEOUT   : SetLastError(WAIT_TIMEOUT,   CErrorStr_WaitTimeout  );
        WAIT_ABANDONED : SetLastError(WAIT_ABANDONED, CErrorStr_WaitAbandoned);
...
begin
  ...
  if CheckValid then
    if FProcess = GetCurrentProcessID then begin
      ...
      if not result then SetLastError(WAIT_FAILED, CErrorStr_WaitFailed);
      ...
      if ih1.IsValid then begin
        ...
        if ih2.IsValid then begin
          ...
          if DuplicateHandle(ih1.Handle, FHandle, ih2.Handle, @handle, access, inheritHandles, c1) then
               result := TIHandle.Create(true, 0, '', targetProcess.ID, handle, autoClose, nil, nil)
          else SetLastError(GetLastError);
        end else SetLastError(ih2.LastErrorNo, ih2.LastErrorStr);
      end else SetLastError(ih1.LastErrorNo, ih1.LastErrorStr);
    end;
    if result = nil then
      result := TIHandle.Create(false, FLastErrorNo, GetLastErrorStr, 0, 0, false, nil, nil);
  end else result := TIHandle.Create(false, ERROR_INVALID_PARAMETER, '', 0, 0, false, nil, nil);
end;
...
begin
  result := TIHandle.Create(true, 0, '', GetCurrentProcessID, handle, autoClose, nil, nil);
end;

// ***************************************************************

constructor TIHandles.Create(valid: boolean; lastErrorNo: cardinal; lastErrorStr: string;
...
begin
  if FProcess = nil then
    FProcess := TIProcess.Create(false, CErrorNo_AmInvalid, CErrorStr_AmInvalid, otProcess, 0);
  result := FProcess;
end;
...
begin
  if FObj = nil then
    FObj := TIOtherKernelObj.Create(false, CErrorNo_AmInvalid, CErrorStr_AmInvalid, otUnknown, 0);
  result := FObj;
end;
...
begin
  if (index < 0) or (index >= FCount) then
       result := TIHandle.Create(false, CErrorNo_IndexOutOfRange, CErrorStr_IndexOutOfRange, 0, 0, false, nil, nil)
...
var oldCount : integer;
  ...
  begin
    if (FObj = nil) or (FObj.ObjAddr = addr) then begin
      ...
      ih1 := TIHandle.Create(true, 0, '', process, handle, false, hi95, hiNt);
  ...
  if CheckValid then
    ...
    {$else}         case madKernel.WaitFor   (FItems, waitAll, 0, false) of {$endif}
      ...
      WAIT_ABANDONED .. (WAIT_ABANDONED + MAXIMUM_WAIT_OBJECTS - 1):
        SetLastError(WAIT_ABANDONED, CErrorStr_WaitAbandoned);
...
begin
  ...
  try
    with waitForRec do begin
      ...
      ready := true;
      PostThreadMessage(waitingThreadID, WM_NULL, 0, 0);
    end;
  except sysUtils.ShowException(ExceptObject, ExceptAddr) end;
end;
    ...
    try
      with notifyRec^ do begin
        ...
        until InFinalization or (result_ = integer(WAIT_FAILED)) or
              (SendMessage(window_, msg_, result_, integer(handles)) = 0);
      end;
...
type
  ...
  TWndProcRec = record
    eventName1   : array [0..39] of char;
    eventHandle1 : cardinal;
    eventName2   : array [0..39] of char;
...
constructor TIWindow.Create(valid: boolean; lastErrorNo: cardinal; lastErrorStr: string;
                            handle, tid, pid: cardinal);
begin
  inherited Create(valid, lastErrorNo, lastErrorStr);
  if FValid then begin
...
begin
  if FThread = nil then
    ...
    else FThread := TIThread.Create(false, CErrorNo_AmInvalid, CErrorStr_AmInvalid, otUnknown, 0);
  result := FThread;
end;
...
begin
  if FProcess = nil then
    ...
    else FProcess := TIProcess.Create(false, CErrorNo_AmInvalid, CErrorStr_AmInvalid, otUnknown, 0);
  result := FProcess;
end;
...
function TIWindow.ChildWindow(className : string;
                              caption   : string = '*') : IWindow;
var pc1, pc2 : pchar;
    c1       : cardinal;
begin
  if CheckValid then begin
    if className <> '*' then pc1 := pchar(className) else pc1 := nil;
    if caption   <> '*' then pc2 := pchar(caption  ) else pc2 := nil;
    c1 := FindWindowEx(FHandle, 0, pc1, pc2);
    if c1 <> 0 then result := AddWindow(c1)
    else            result := TIWindow.Create(false, GetLastError, '', 0, 0, 0);
  end else result := TIWindow.Create(false, FLastErrorNo, FLastErrorStr, 0, 0, 0);
end;
...
function TIWindow.GetProp(name: string) : cardinal;
begin
  if CheckValid then begin
    result := windows.GetProp(FHandle, pchar(name));
    if result = 0 then
      SetLastError(ERROR_FILE_NOT_FOUND);
  end else result := 0;
end;

procedure TIWindow.SetProp(name: string; value: cardinal);
begin
  if CheckValid then begin
    if not windows.SetProp(FHandle, pchar(name), value) then
      SetLastError(CErrorNo_Unknown, CErrorStr_Unknown);
  end;
end;

function TIWindow.DeleteProp(name: string) : boolean;
begin
  if CheckValid then begin
    result := RemoveProp(FHandle, pchar(name)) = 0;
    if not result then
      SetLastError(ERROR_FILE_NOT_FOUND);
  end else result := false;
end;
...
begin
  if CheckValid then begin
    result := pointer(windows.GetProp(FHandle, pchar('Delphi' + IntToHex(FProcessID, 8))));
    if result = nil then
      SetLastError(ERROR_FILE_NOT_FOUND);
  end else result := nil;
end;
...
begin
  if CheckValid then begin
    result := windows.PostMessage(FHandle, msg, wParam, lParam);
    if not result then
      SetLastError(GetLastError);
  end else result := false;
end;
...
begin
  if CheckValid then begin
    ...
    windows.SetLastError(0);
    if SendMessageTimeout(FHandle, msg, wParam, lParam, c1, timeOut, cardinal(result)) = 0 then begin
      result := 0;
      if GetLastError = 0 then SetLastError(WAIT_TIMEOUT, CErrorStr_WaitTimeout)
      else                     SetLastError(GetLastError);
    end;
  end else result := 0;
end;
...
begin
  if CheckValid then begin
    result := SendMessageCallback(FHandle, msg, wParam, lParam, @callbackProc, callbackInfo);
    if not result then
      SetLastError(GetLastError);
  end else result := false;
end;
...
begin
  if CheckValid then begin
    result := windows.PostMessage(FHandle, WM_CLOSE, 0, 0);
    if not result then
      SetLastError(GetLastError);
  end else result := false;
end;
...
function TIWindow.GetClassName : string;
begin
  if (FClassName = '') and CheckValid then begin
    SetLength(FClassName, MAX_PATH + 1);
    SetLength(FClassName, windows.GetClassName(FHandle, pchar(FClassName), MAX_PATH));
  end;
  result := FClassName;
end;
...
constructor TIWindows.Create(valid: boolean; lastErrorNo: cardinal; lastErrorStr: string;
                             const thread: IThread; const process: IProcess; inTaskbar: boolean;
                             const objects: array of IBasic);
var i1 : integer;
begin
...
begin
  if (index < 0) or (index >= FCount) then
       result := TIWindow.Create(false, CErrorNo_IndexOutOfRange, CErrorStr_IndexOutOfRange, 0, 0, 0)
...
begin
  ...
  if CheckValid then
    ...
    end else SetLastError(CErrorNo_RefreshListError, CErrorStr_RefreshListError);
end;
...
type
  ...
  public
    ...
    constructor Create (valid: boolean; lastErrorNo: cardinal; lastErrorStr: string;
                        trayIcons: TITrayIcons; const nid: TNotifyIconData; imageIndex: integer);
    destructor Destroy; override;
...
constructor TITrayIcon.Create(valid: boolean; lastErrorNo: cardinal; lastErrorStr: string;
                              trayIcons: TITrayIcons; const nid: TNotifyIconData; imageIndex: integer);
begin
  inherited Create(valid, lastErrorNo, lastErrorStr);
  if FValid then begin
...
begin
  if FWindow = nil then
    ...
    else FWindow := TIWindow.Create(false, CErrorNo_AmInvalid, CErrorStr_AmInvalid, 0, 0, 0);
  result := FWindow;
end;
...
function TITrayIcon.GetHint : string;
begin
  result := FNID.szTip;
end;

procedure TITrayIcon.SetHint(value: string);
var s1 : string;
begin
  if CheckValid then begin
    s1 := FNID.szTip;
...
function TITrayIcon.GetRect : TRect;
  ...
  function FindToolbarWindow32(window: dword; var tb: dword) : bool; stdcall;
  var arrCh : array [0..MAX_PATH] of char;
  begin
    GetClassName(window, arrCh, MAX_PATH);
    result := arrCh <> 'ToolbarWindow32';
    if not result then
      tb := window;
  end;
    ...
    with TrayWindow do
      if IsValid then begin
        if FTrayIcons = nil then begin
          ...
          ix := self.GetIndex(FTrayIcons);
        tb := 0;
        EnumChildWindows(Handle, @FindToolbarWindow32, integer(@tb));
        if tb <> 0 then begin
          for i1 := windows.SendMessage(tb, TB_BUTTONCOUNT, 0, 0) - 1 downto 0 do begin
            state := dword(windows.SendMessage(tb, TB_GETSTATE, i1, 0));
            if (state <> dword(-1)) and (state and TBSTATE_HIDDEN = 0) then
              if ix = 0 then begin
                with OwnerProcess do
                  if IsValid then
                    with GetHandle(PROCESS_QUERY_INFORMATION or PROCESS_VM_READ or
                                   PROCESS_VM_WRITE or PROCESS_VM_OPERATION) do
                      if IsValid then
                        try
                          prec := AllocMem(sizeOf(TRect), true);
                          try
                            if (windows.SendMessage(tb, TB_GETRECT, i1, integer(prec)) <> 0) and
                               ReadMemory(prec^, result, sizeOf(TRect)) and
                               GetWindowRect(tb, rect1) then 
                              OffsetRect(result, rect1.Left, rect1.Top);
                          finally FreeMem(pointer(prec)) end;
                        except end;
                break;
              end else
                dec(ix);
          ...
          with ClientRect do
            ih := Bottom - Top - 3;
          with Rect do begin
            result.Left   := Left + 1 + ih * ix;
            result.Right  := result.Left + ih;
            result.Top    := Top + 2;
            result.Bottom := Top + 2 + ih;
          end;
        end;
      end else self.SetLastError(CErrorNo_Unknown, CErrorStr_Unknown);
end;

procedure TITrayIcon.MouseMove;
begin
  if CheckValid then
    PostMessage(FNID.Wnd, FNID.uCallbackMessage, FNID.uID, WM_MOUSEMOVE);
end;
...
begin
  if CheckValid then
    if double then begin
      if right then
           PostMessage(FNID.Wnd, FNID.uCallbackMessage, FNID.uID, WM_RBUTTONDBLCLK)
      else PostMessage(FNID.Wnd, FNID.uCallbackMessage, FNID.uID, WM_LBUTTONDBLCLK);
    end else
      if right then begin
        PostMessage(FNID.Wnd, FNID.uCallbackMessage, FNID.uID, WM_RBUTTONDOWN);
        PostMessage(FNID.Wnd, FNID.uCallbackMessage, FNID.uID, WM_RBUTTONUP  );
      end else begin
        PostMessage(FNID.Wnd, FNID.uCallbackMessage, FNID.uID, WM_LBUTTONDOWN);
        PostMessage(FNID.Wnd, FNID.uCallbackMessage, FNID.uID, WM_LBUTTONUP  );
      end;
end;
...
constructor TITrayIcons.Create(valid: boolean; lastErrorNo: cardinal; lastErrorStr: string;
                               const process: IProcess; const objects: array of IBasic);
var i1 : integer;
begin
  inherited Create(valid, lastErrorNo, lastErrorStr);
...
begin
  if FProcess = nil then
    FProcess := TIProcess.Create(false, CErrorNo_AmInvalid, CErrorStr_AmInvalid, otUnknown, 0);
  result := FProcess;
end;
...
begin
  if (index < 0) or (index >= FCount) then
       result := TITrayIcon.Create(false, CErrorNo_IndexOutOfRange, CErrorStr_IndexOutOfRange, nil, nid, 0)
...
type
  ...
  procedure RefreshItem(nid: TNotifyIconData; imageIndex: integer);
  var i1   : integer;
      iti1 : TITrayIcon;
      pid  : cardinal;
  begin
    ...
    if FAll or (pid = FProcess.ID) then begin
        ...
        with TITrayIcon(FItems[i1].SelfAsTObject) do
          if (FNID.uID = nid.uID) and (FNID.Wnd = nid.Wnd) then begin
            if (FNID.uFlags           <> nid.uFlags          ) or
               ...
               (string(FNID.szTip)    <> string(nid.szTip)   ) or
      ...
      iti1 := TITrayIcon.Create(true, 0, '', self, nid, imageIndex);
  ...
  begin
    ...
    if OS.winNT then begin
      ...
      try
        ...
        GetModuleFileName(GetModuleHandle('comctl32.dll'), arrCh, MAX_PATH);
      ...
      end else SetLastError(CErrorNo_Unknown, CErrorStr_Unknown);
    end;
  end;
                            ...
                            if WriteMemory(tic, ptic^, sizeOf(TTrayIconCommand2000Ex)) then begin
                              ...
                              if ReadMemory(ptic^, tic, sizeOf(TTrayIconCommand2000Ex)) and
                                ...
                                if tic.main.state and 8 = 0 then begin  // not hidden
                                  ...
                                  StrPLCopy(tii.notifyIconDataA.szTip, wideString(tic.str), 64);
                                  tii.notifyIconDataA.cbSize := sizeOf(TNotifyIconDataA);
                                  RefreshItem(tii.notifyIconDataA, tic.main.imageIndex);
                                end;
                              end;
                            end;
                          end;
                        ...
                        if ReadMemory(twi.iconsInfo^, tisi, sizeOf(TTrayIconsInfo)) and
                          ...
                          for i1 := 0 to tisi.IconCount - 1 do
                            if ReadMemory(pointer(integer(tisi.trayIconInfos) + i1 * 4)^, p1, 4) and
                              ...
                              tii.notifyIconDataA.cbSize := sizeOf(TNotifyIconDataA);
                              RefreshItem(tii.notifyIconDataA, tii.imageIndex);
                            end;
                          result := true;
                        end;
                      end;
...
type
  ...
  public
    ...
    constructor Create (valid: boolean; lastErrorNo: cardinal; lastErrorStr: string;
                        pid: cardinal; moduleName: string; moduleHandle: cardinal;
                        ordinal: cardinal; name: string; address: pointer);
  ...
  public
    ...
    constructor Create (valid: boolean; lastErrorNo: cardinal; lastErrorStr: string;
                        ...
                        handle: cardinal; fileName_: string; autoClose: boolean);
    destructor Destroy; override;
...
function FindXxportList(import: boolean; moduleName: string; moduleHandle: cardinal) : IXxportList;
var i1 : integer;
begin
  result := nil;
...
procedure AddXxportList(const list: IXxportList; import: boolean;
                        moduleName: string; moduleHandle: cardinal);
var i1 : integer;
    b1 : boolean;
begin
...
constructor TIExportEntry.Create(valid: boolean; lastErrorNo: cardinal; lastErrorStr: string;
                                 pid: cardinal; moduleName: string; moduleHandle: cardinal;
                                 ordinal: cardinal; name: string; address: pointer);
begin
  inherited Create(valid, lastErrorNo, lastErrorStr);
...
begin
  ...
  else result := TIModule.Create(false, CErrorNo_AmInvalid, CErrorStr_AmInvalid, nil, 0, '', false);
end;
...
function TIExportEntry.GetName : string;
begin
  result := FName;
end;
...
begin
  if (index < 0) or (index >= FCount) then
       result := TIExportEntry.Create(false, CErrorNo_IndexOutOfRange, CErrorStr_IndexOutOfRange, 0, '', 0, 0, '', nil)
...
constructor TIModule.Create(valid: boolean; lastErrorNo: cardinal; lastErrorStr: string;
                            const process: IProcess;
                            handle: cardinal; fileName_: string; autoClose: boolean);
var i1  : integer;
    fno : string;  // file name only
    b1  : boolean;
begin
  inherited Create(valid, lastErrorNo, lastErrorStr);
  if FValid then begin
    ...
    if FValid then begin
      ...
      if FHandle = 0 then
        if FProcess.ID = GetCurrentProcessID then begin
          FHandle   := GetModuleHandle(pchar(FFileName));
          FFileName := '';
          if FHandle = 0 then SetLastError(GetLastError);
        end else begin
          with process.Modules do
            if ExtractFilePath(fileName_) <> '' then begin
              fno := ExtractFileName(fileName_);
              for i1 := 0 to ItemCount - 1 do begin
                b1 := Pos('\', Items[i1].FileName) = 0;
              ...
              for i1 := 0 to ItemCount - 1 do
                if IsTextEqual(fileName_, ExtractFileName(Items[i1].FileName)) then begin
                  FHandle := Items[i1].Handle;
                  break;
                end;
      ...
      if FValid and (FFileName = '') then
        if FProcess.ID = GetCurrentProcessID then begin
          ...
          if GetModuleFileName(FHandle, pchar(FFileName), MAX_PATH) = 0 then begin
            FFileName := '';
            SetLastError(GetLastError);
          end else FFileName := pchar(FFileName);
        end else begin
          with process.Modules do
            for i1 := 0 to ItemCount - 1 do
...
begin
  if FValid then
    if FProcess.ID = GetCurrentProcessID then begin
      FValid := GetModuleHandle(pchar(FFileName)) = FHandle;
    end else begin
      FValid := false;
...
function TIModule.GetFileName : string;
begin
  result := FFileName;
end;
...
function TIModule.GetProcAddress(name: string) : pointer;
              ...
              with exp do begin
                ...
                if ReadMemory(pointer(FHandle + AddressOfNames       )^, pointer(aon )^, NumberOfNames     * 4) and
                  ...
                  for i1 := 0 to NumberOfNames - 1 do
                    if ReadMemory(pchar(FHandle + aon[i1])^, arrCh, MAX_PATH) and (arrCh = name) then begin
                      c1 := aof[aono[i1]];
                      if c1 > 0 then
                        with inh.OptionalHeader.DataDirectory[IMAGE_DIRECTORY_ENTRY_EXPORT] do
                          if (c1 >= VirtualAddress) and (c1 < VirtualAddress + Size) then begin
                            if ReadMemory(pchar(FHandle + c1)^, arrCh, MAX_PATH) then begin
                              pc1 := arrCh;
                              pc2 := pc1;
                              repeat inc(pc2) until pc2^ = '.';
                              pc2^ := #0;
                              inc(pc2);
                              result := Module(string(pc1) + '.dll').GetProcAddress(pc2);
                            end;
                          end else
  ...
  if CheckValid then begin
    ...
    if pexp <> nil then
      if (FProcess.ID = GetCurrentProcessID) or (OS.win9x and (FHandle >= $80000000)) then begin
        ...
        if result = nil then
          result := windows.GetProcAddress(FHandle, pchar(index));
        if result = nil then
          SetLastError(GetLastError);
      end else
        with FProcess, GetHandle(PROCESS_VM_READ) do
...
function TIModule.SetProcAddress(name: string; newAddr: pointer) : boolean;
var pexp : PImageExportDirectory;
    exp  : TImageExportDirectory;
    i1   : integer;
    s1   : string;
    ...
    if pexp <> nil then
      if (FProcess.ID = GetCurrentProcessID) or (OS.win9x and (FHandle >= $80000000)) then begin
        try
          with pexp^ do
            for i1 := 0 to NumberOfNames - 1 do
              if pchar(FHandle + TPACardinal(FHandle + AddressOfNames)^[i1]) = name then begin
            ...
            if ReadMemory(pexp^, exp, sizeOf(TImageExportDirectory)) then
              with exp do begin
                ...
                if ReadMemory(pointer(FHandle + AddressOfNames       )^, pointer(aon )^, NumberOfNames     * 4) and
                  ...
                  for i1 := 0 to NumberOfNames - 1 do
                    if ReadMemory(pchar(FHandle + aon[i1])^, pointer(s1)^, MAX_PATH) and
                       (pchar(s1) = name) then begin
...
begin
  if FImportList = nil then
    ...
    end else FImportList := TIXxportList.Create(false, CErrorNo_AmInvalid, CErrorStr_AmInvalid);
  result := FImportList;
end;
...
begin
  if FExportList = nil then
                    ...
                    for i2 := 0 to NumberOfNames - 1 do
                      if TPAWord(FHandle + AddressOfNameOrdinals)^[i2] = i1 then begin
                        ...
                        xl.NewItem(FProcess.ID, FFileName, FHandle, i1 + integer(Base),
                                   pchar(FHandle + TPACardinal(FHandle + AddressOfNames)^[i2]),
                                   pointer(FHandle + c1));
                        break;
                      end;
                    if b1 then
                    ...
                    if ReadMemory(pointer(FHandle + AddressOfNames       )^, pointer(aon )^, NumberOfNames     * 4) and
                      ...
                      for i1 := 0 to NumberOfFunctions - 1 do begin
                        ...
                        if c1 > 0 then begin
                          ...
                          for i2 := 0 to NumberOfNames - 1 do
                            if (aono[i2] = i1) and
                               ReadMemory(pchar(FHandle + aon[i2])^, pointer(s1)^, MAX_PATH) then begin
                              b1 := false;
                              xl.NewItem(FProcess.ID, FFileName, FHandle,
                                         i1 + integer(Base), pchar(s1), pointer(FHandle + c1));
                              break;
                            end;
                          if b1 then
                            xl.NewItem(FProcess.ID, FFileName, FHandle,
    ...
    end else FExportList := TIXxportList.Create(false, CErrorNo_AmInvalid, CErrorStr_AmInvalid);
  result := FExportList;
end;
...
begin
  ...
  c1 := LoadLibraryEx(pchar(fileName), 0, c1);
  if c1 = 0 then
...
type
  ...
  public
    ...
    constructor Create (valid: boolean; lastErrorNo: cardinal; lastErrorStr: string;
                        const process: IProcess; fileName: string; const objects: array of IBasic);
...
constructor TIModules.Create(valid: boolean; lastErrorNo: cardinal; lastErrorStr: string;
                             const process: IProcess; fileName: string; const objects: array of IBasic);
var i1 : integer;
begin
  inherited Create(valid, lastErrorNo, lastErrorStr);
  if FValid then begin
    ...
    FProcess  := process;
    FFileName := fileName;
    FOnlyName := ExtractFilePath(FFileName) = '';
    FList     := Length(objects) > 0;
    if FList then
      for i1 := 0 to high(objects) do
        if objects[i1] <> nil then
...
function TIModules.GetFileName : string;
begin
  result := FFileName;
end;
...
begin
  if (index < 0) or (index >= FCount) then
       result := TIModule.Create(false, CErrorNo_IndexOutOfRange, CErrorStr_IndexOutOfRange, nil, 0, '', false)
  else result := IModule(FItems[index]);
end;

function TIModules.RefreshItems : boolean;

  procedure RefreshItem(const ownerProcess: IProcess; handle: cardinal; fileName: string);
  var s1, s2 : string;
      i1     : integer;
      im1    : TIModule;
  begin
    if FFileName <> '' then begin
      s1 := FFileName;
      s2 := fileName;
      if      FOnlyName                then s2 := ExtractFileName(s2)
      else if ExtractFilePath(s2) = '' then s1 := ExtractFileName(s1);
    end;
                  ...
                  if FirstModuleInfoNt(Handle, mi, loopEnd) then
                    repeat
                      if ModuleInfoNameNt(Handle, mi, ws) then
                        RefreshItem(Items[i1], mi.handle, ws);
          ...
          with FProcess.GetHandle(PROCESS_QUERY_INFORMATION or PROCESS_VM_READ) do
            if IsValid then begin
              if FirstModuleInfoNt(Handle, mi, loopEnd) then
                repeat
                  if ModuleInfoNameNt(Handle, mi, ws) then
                    RefreshItem(FProcess, mi.handle, ws);
                until (mi.next = loopEnd) or (not NextModuleInfoNt(Handle, mi));
            end else self.SetLastError(LastErrorNo, LastErrorStr);
      result := true;
    except end;
...
function Modules(fileName: string) : IModules;
begin
...
begin
  if CheckValid then begin
    ...
    if FHandle = nil then
      FHandle := TIHandle.Create(false, CErrorNo_AmInvalid, CErrorStr_AmInvalid,
                                 0, 0, false, nil, nil);
    result := FHandle;
  end;
end;
  ...
  if CheckValid then begin
    if (FHandle = nil) or (FHandle.Access and access <> access) or
      ...
      result := TIHandle.Create(c4 <> 0, FLastErrorNo, GetLastErrorStr,
                                GetCurrentProcessID, c4, true, nil, nil);
...
begin
  result := TIWindows.Create(FValid, CErrorNo_AmInvalid, CErrorStr_AmInvalid, self, FProcess, false, []);
end;

function TIThread.GetTaskbarWindows : IWindows;
begin
  result := TIWindows.Create(FValid, CErrorNo_AmInvalid, CErrorStr_AmInvalid, self, FProcess, true, []);
end;
...
begin
  if CheckValid then begin
    result := PostThreadMessage(GetID, msg, wParam, lParam);
    if not result then SetLastError(GetLastError);
  end else result := false;
end;
...
begin
  if CheckValid then begin
    result := PostThreadMessage(GetID, WM_QUIT, exitCode, 0);
    if not result then SetLastError(GetLastError);
  end else result := false;
end;
...
constructor TIThreads.Create(valid: boolean; lastErrorNo: cardinal; lastErrorStr: string;
                             const process: IProcess; const objects: array of IBasic);
var i1 : integer;
begin
  inherited Create(valid, lastErrorNo, lastErrorStr);
...
begin
  if (index < 0) or (index >= FCount) then
       result := TIThread.Create(false, CErrorNo_IndexOutOfRange, CErrorStr_IndexOutOfRange, otThread, 0)
...
begin
  ...
  if CheckValid then
    for i1 := 0 to FCount - 1 do
      result := PostThreadMessage(IThread(FItems[i1]).ID, msg, wParam, lParam) or result;
end;
...
function GetProcessFileNameNt(handle: cardinal) : string;
var pbi  : TNtProcessBasicInfo;
    c1   : cardinal;
    pp   : pointer;
    ws   : wideString;
...
function TIProcess.GetExeFile : string;
var i1 : integer;
begin
...
begin
  if CheckValid then begin
    if (FHandle = nil) or (FHandle.Access and access <> access) or
      ...
      if c1 = 0 then begin
        SetLastError(GetLastError);
        result := TIHandle.Create(false, GetLastError, '', 0, 0, false, nil, nil);
      end else
        result := TIHandle.Create(true, 0, '', GetCurrentProcessID, c1, true, nil, nil);
      if result.IsValid and (OS.winNT or (FStore > 0)) then
        FHandle := result;
    end else result := FHandle;
  end else begin
...
begin
  result := TIModule.Create(FValid, CErrorNo_AmInvalid, CErrorStr_AmInvalid, self, GetHInstance, '', false);
end;
...
function TIProcess.GetServiceProcess : boolean;
  ...
  begin
    result := true;
    saa := nil;
    if GetProcessSid(processHandle, saa) then begin
      if (saa <> nil) and (saa^.Sid <> nil) and (TPAByte(saa^.Sid)^[1] > 1) then
        result := false;
      if saa <> nil then
        LocalFree(dword(saa));
    end;
    ...
    if OS.win9x then begin
      try
        ...
        if not result then begin
          ...
          if IsTextEqual(string(arrCh) + '\' + kernel32, GetExeFile) then
            result := true;
        end;
      except end;
    end else
...
begin
  ...
  if CheckValid then
    with GetHandle2(PROCESS_QUERY_INFORMATION) do
      if IsValid then begin
        if not GetProcessAffinityMask(Handle, result, c1) then begin
          result := c1;
          self.SetLastError(GetLastError);
        end;
      end else self.SetLastError(LastErrorNo, LastErrorStr);
...
function TIProcess.GetCommandLine : string;
var b1 : boolean;
begin
...
begin
  result := TIThreads.Create(FValid, CErrorNo_AmInvalid, CErrorStr_AmInvalid, self, []);
end;
...
function TIProcess.Module(handle    : cardinal;
                          autoClose : boolean = false) : IModule;
begin
  result := TIModule.Create(FValid, CErrorNo_AmInvalid, CErrorStr_AmInvalid, self, handle, '', autoClose);
end;
...
begin
  with GetHandle2(PROCESS_QUERY_INFORMATION) do
    if IsValid and (VirtualQueryEx(Handle, memory, mbi, sizeOf(mbi)) = sizeOf(mbi)) and
       (mbi.State = MEM_COMMIT) and (mbi.AllocationBase <> nil) then
      memory := mbi.AllocationBase;
  result := TIModule.Create(FValid, CErrorNo_AmInvalid, CErrorStr_AmInvalid, self, cardinal(memory), '', autoClose);
end;

function TIProcess.Module(fileName  : string;
                          autoClose : boolean = false) : IModule;
begin
  result := TIModule.Create(FValid, CErrorNo_AmInvalid, CErrorStr_AmInvalid, self, 0, fileName, autoClose);
end;

function TIProcess.LoadModule(fileName          : string;
                              autoClose         : boolean = true;
                              withoutReferences : boolean = false;
                              onlyAsDataFile    : boolean = false;
                              alteredSearchPath : boolean = false;
      ...
      for i1 := 0 to ItemCount - 1 do
        with Items[i1].ExportList do
          for i2 := 0 to ItemCount - 1 do
            xl.AddItem(Items[i2]); 
  end else result := TIXxportList.Create(false, CErrorNo_AmInvalid, CErrorStr_AmInvalid);
end;

function TIProcess.GetModules : IModules;
begin
  result := TIModules.Create(FValid, CErrorNo_AmInvalid, CErrorStr_AmInvalid, self, '', []);
end;

function TIProcess.GetTrayIcons : ITrayIcons;
begin
  result := TITrayIcons.Create(FValid, CErrorNo_AmInvalid, CErrorStr_AmInvalid, self, []);
end;

function TIProcess.GetWindows : IWindows;
begin
  result := TIWindows.Create(FValid, CErrorNo_AmInvalid, CErrorStr_AmInvalid, nil, self, false, []);
end;

function TIProcess.GetTaskbarWindows : IWindows;
begin
  result := TIWindows.Create(FValid, CErrorNo_AmInvalid, CErrorStr_AmInvalid, nil, self, true, []);
end;
...
begin
  result := TIHandles.Create(FValid, CErrorNo_AmInvalid, CErrorStr_AmInvalid, self, nil, []);
end;
    ...
    if fi.IsValid then begin
      if fi.Copy.IsValid then begin
        if acceptUnknownTargets or (fi.UnknownTargets = nil) then begin
          ...
          raise Exception.Create(CErrorStr_UnknownTarget) at fi.UnknownTargets[0].CodeAddr2;
      end else
        raise Exception.Create(fi.Copy.LastErrorStr) at fi.Copy.LastErrorAddr;
    end else
      raise Exception.Create(fi.LastErrorStr) at fi.LastErrorAddr;
  end;
...
begin
  ...
  for i1 := 0 to raHigh do
    if replace[i1] = nil then begin
      self.SetLastError(CErrorNo_InvalidReplaceArray, CErrorStr_InvalidReplaceArray);
      exit;
    end;
  with GetHandle2(PROCESS_VM_WRITE or PROCESS_VM_OPERATION) do
    if IsValid then begin
  ...
  with TInjectRec(parameter^) do
    while WaitForSingleObject(event1, INFINITE) = WAIT_OBJECT_0 do begin
      if command = icExitProcess then begin
        ...
        params[0] := LoadLibraryEx(name, 0, params[0]);
        result := params[0] <> 0;
        ...
        pc1 := GetCommandLine;
        pc2 := cmdLine;
        while (pc1^ <> #0) and (pc2 - pchar(@cmdLine) < MAX_PATH) do begin
          pc2^ := pc1^;
          inc(pc1);
          inc(pc2);
        end;
  ...
  if CheckValid then
    if GetExeType in [etWin32, etConsole] then begin
      if FInjectMutex = nil then
        FInjectMutex := NewMutex('InjectMutex $' + IntToHex(FID, 1), false);
          ...
          with GetHandle2(PROCESS_QUERY_INFORMATION or PROCESS_VM_READ or PROCESS_CREATE_THREAD or
            ...
            if IsValid then begin
              if FInjectNB <> nil then begin
                ...
                FInjectNB := OpenNamedBuffer('InjectNB $' + IntToHex(FID, 1), false);
                if FInjectNB.IsValid then begin
                  FInjectMem := TPPointer(FInjectNB.Memory)^;
                  FInjectEvent1 := OpenEvent('InjectEvent1 $' + IntToHex(FID, 1));
                  FInjectEvent2 := OpenEvent('InjectEvent2 $' + IntToHex(FID, 1));
                  b1 := true;
                  b2 := true;
                end else begin
                  b1 := false;
                  ...
                  if FInjectMem <> nil then begin
                    if CopyCode(@InjectThread, @InjectThread_End, @TPInjectRecEx(FInjectMem)^.injectCode,
                                [@WaitForSingleObject,    kdll.GetProcAddress('WaitForSingleObject'),
                                 ...
                                 @LoadLibraryEx,          kdll.GetProcAddress('LoadLibraryExA'     ),
                                 @FreeLibrary,            kdll.GetProcAddress('FreeLibrary'        ),
                                 @windows.GetCommandLine, kdll.GetProcAddress('GetCommandLineA'    ),
                      ...
                      FInjectEvent1 := NewEvent(true, true, 'InjectEvent1 $' + IntToHex(FID, 1));
                      FInjectEvent2 := NewEvent(true, true, 'InjectEvent2 $' + IntToHex(FID, 1));
                      ...
                      if WriteMemory(ir, FInjectMem^, sizeOf(TInjectRec)) then
                        if it.Resume then begin
                          FInjectNB := NewNamedBuffer('InjectNB $' + IntToHex(FID, 1), 4);
              ...
              if b1 and FInjectEvent1.UnLock and FInjectEvent2.WaitFor(timeOut, false) and
                ...
                if command = icGetCommandLine then begin
                  result := true;
                  if result_ <> nil then TPString(result_)^ := ir.cmdLine;
                end else if command = icGetStartupInfo then begin
                  result := true;
    ...
    end else SetLastError(CErrorNo_NoWin32Process, CErrorStr_NoWin32Process);
end;
      ...
      if FID <> GetCurrentProcessID then begin
        {$ifdef madDisAsm}
          with GetHandle2(PROCESS_QUERY_INFORMATION or PROCESS_VM_READ or PROCESS_CREATE_THREAD or
                          PROCESS_VM_WRITE or PROCESS_VM_OPERATION or PROCESS_DUP_HANDLE) do
            if IsValid then begin
              with CopyFunction(@func, paramsSize, acceptUnknownTargets) do
                if IsValid then
                  try
    ...
    with GetHandle2(PROCESS_QUERY_INFORMATION) do
      if IsValid then begin
        case windows.WaitForInputIdle(Handle, timeOut) of
          0            : result := true;
          WAIT_TIMEOUT : self.SetLastError(WAIT_TIMEOUT, CErrorStr_WaitTimeout);
          else           self.SetLastError(GetLastError);
        end;
      end else self.SetLastError(LastErrorNo, LastErrorStr);
end;
...
function Process(exeFile: string) : IProcess;
begin
...
function NewProcess(exeFile        : string;
  ...
  function GetParamStr(pc: pchar; var param: string) : pchar;
  var len : integer;
  ...
  function ParamCount(cmdLine: pchar = nil) : integer;
  var s1 : string;
  begin
    if cmdLine = nil then cmdLine := GetCommandLine;
  ...
  function ParamStr(index: integer; cmdLine: pchar = nil): string;
  var buf : array [0..260] of char;
  begin
    if cmdLine = nil then cmdLine := GetCommandLine;
    ...
    end else SetString(result, buf, GetModuleFileName(0, buf, sizeOf(buf)));
  end;
...
begin
  ...
  s1 := ExtractFileExt(exeFile);
  if IsTextEqual(s1, '.lnk') or IsTextEqual(s1, '.pif') then begin
    ...
    if (CoCreateInstance(CLSID_ShellLink, nil, CLSCTX_INPROC_SERVER or CLSCTX_LOCAL_SERVER, IShellLink, sl) = 0) and
       ...
       (pf.Load(PWideChar(wideString(exeFile)), 0) = 0) and
  ...
  if ExeType(exeFile) in [etDos, etWin16] then
    if params <> '' then begin
      s1 := '"x" ' + params;
      params := '';
      for i1 := 1 to ParamCount(pchar(s1)) do begin
        s2 := ParamStr(i1, pchar(s1));
        s3 := s2;
        KillChar(s3, '"');
        if GetFileAttributes(pchar(s3)) <> dword(-1) then begin
          s4 := GetShortFileName(s3);
          if (s4 <> s3) and (Pos(' ', s4) = 0) then
               params := params + ' ' + s4
  ...
  s1 := '"' + exeFile + '"';
  if params <> '' then s1 := s1 + ' ' + params;
  if CreateProcess(nil, pchar(s1), processAttr, threadAttr, false,
                   creationFlags, environment, pc1, si, pi) then begin
    ...
    if pi.dwProcessID <> 0 then begin
      ...
      result := TIProcess.Create(false, CErrorNo_ProcessStartedButNoID,
                                 CErrorStr_ProcessStartedButNoID, otUnknown, 0);
    end;
  end else
...
constructor TIProcesses.Create(valid: boolean; lastErrorNo: cardinal; lastErrorStr: string;
                               exeFile: string; const objects: array of IBasic);
var i1 : integer;
begin
  inherited Create(valid, lastErrorNo, lastErrorStr);
...
function TIProcesses.GetExeFile : string;
begin
  result := FExeFile;
end;
...
begin
  if (index < 0) or (index >= FCount) then
       result := TIProcess.Create(false, CErrorNo_IndexOutOfRange, CErrorStr_IndexOutOfRange, otProcess, 0)
  else result := IProcess(FItems[index]);
end;

function TIProcesses.RefreshItems : boolean;

  procedure RefreshItem(pid, parentPid, session: cardinal; exeFile: string);
  var i1     : integer;
      s1, s2 : string;
      b1     : boolean;
      ip1    : TIProcess;
  begin
    if not FAll then begin
      s1 := FExeFile;
      s2 := exeFile;
      if      ExtractFilePath(s1) = '' then s2 := ExtractFileName(s2)
      else if ExtractFilePath(s2) = '' then s1 := ExtractFileName(s1);
...
function Processes(exeFile: string = '*') : IProcesses;
begin
  if exeFile = '*' then
...
function TIEvent.GetName : string;
begin
  result := FName;
end;
...
var c1  : cardinal;
    ...
    ie1 : TIEvent;
begin
  if name = '' then pc1 := nil else pc1 := pchar(name);
  c1 := windows.OpenEvent(EVENT_ALL_ACCESS, true, pc1);
...
begin
  if name = '' then pc1 := nil else pc1 := pchar(name);
  c1 := CreateEvent(eventAttr, not auto, not locked, pc1);
  ie1 := TIEvent(AddKernelObj(otEvent, Handle(c1)));
  ie1.FName       := name;
...
function TIMutex.GetName : string;
begin
  result := FName;
end;
...
begin
  if name = '' then pc1 := nil else pc1 := pchar(name);
  c1 := windows.OpenMutex(MUTEX_ALL_ACCESS, true, pc1);
...
begin
  if name = '' then pc1 := nil else pc1 := pchar(name);
  c1 := CreateMutex(mutexAttr, bool(integer(enter)), pc1);
...
type
  ...
  public
    ...
    constructor Create (valid: boolean; lastErrorNo: cardinal; lastErrorStr: string;
                        name: string; size: integer; writeAccess: boolean;
...
constructor TINamedBuffer.Create(valid: boolean; lastErrorNo: cardinal; lastErrorStr: string;
                                 name: string; size: integer; writeAccess: boolean;
                                 buffer: pointer; handle: cardinal);
begin
  if valid then
...
function TINamedBuffer.GetName : string;
begin
  result := FName;
end;
...
begin
  if name <> '' then begin
    ...
    c2 := OpenFileMapping(c1, true, pchar(name));
    if c2 <> 0 then begin
...
begin
  if name <> '' then begin
    c1 := OpenFileMapping(FILE_MAP_ALL_ACCESS, true, pchar(name));
    if c1 <> 0 then begin
      ...
      if size > 0 then begin
        ...
        c1 := CreateFileMapping(cardinal(-1), @sa, PAGE_READWRITE, 0, size + 4, pchar(name));
        if c1 <> 0 then begin
          p1 := MapViewOfFile(c1, FILE_MAP_ALL_ACCESS, 0, 0, 0);
          if p1 <> nil then begin
            TPInteger(p1)^ := size;
...
var s1 : string;
  ...
  procedure Add(name: string; hnd: cardinal);
  begin
    s1 := s1 + #$D#$A + name + ': ';
...
begin
  ...
  Add('File', CreateFile(pchar(arrCh + '\regedit.exe'), GENERIC_READ, FILE_SHARE_READ, nil, OPEN_EXISTING, 0, 0));  // 7 8
  ...
  Delete(s1, 1, 2);
  if MessageBox(0, pchar(s1), 'info', MB_YESNO) <> ID_YES then ExitProcess(0);

  with Processes do
//    for i2 := 0 to ItemCount - 1 do
      with CurrentProcess.(*Items[i2],*) Handles do begin
        s1 := ExtractFileName(ExeFile) + ' has ' + IntToStr(ItemCount) + ' Handles' + #$D#$A;
        i1 := ItemCount - 1;
        if i1 > 35 then i1 := 35;
        for i1 := 0 to i1 do
          with TIHandle(Items[i1].SelfAsTObject) do
            s1 := s1 + #$D#$A + 'Handle: ' + IntToHex(FHandle, 1) + '; ' +
                                'Type: ' + GetKernelObj.ObjTypeStr + '/' + CObjNames[FType] + ' (' + IntToStr(FTypeNo) + '); ' +
                                'Access: ' + IntToHex(FAccess, 1);
        if MessageBox(0, pchar(s1), 'info', MB_YESNO) <> ID_YES then ExitProcess(0);
      end;
  MessageBox(0, pchar(string(booleanToChar(Event(CreateEvent(nil, false, false, nil)).IsAutoEvent)) +
