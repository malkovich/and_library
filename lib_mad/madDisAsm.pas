// ***************************************************************
//  madDisAsm.pas             version:  2.1d  ? date: 2006-04-24
//  -------------------------------------------------------------
...
// 2006-04-24 2.1d (1) another little bug in ParseFunction fixed
...
function ParseCode (code: pointer; var imLen, imVal: integer) : TCodeInfo; overload;
...
function NtProc (api: string) : pointer;
...
const
  ...
  COpcodeFlags : array [$00..$ff] of word =
    ((fReg8   + fMod8   +                fOrder +                fClrM ),  // 00 /r       add     r/mb, rb          (r)
     ...
     (fRegxx  + fMod32  + f66RM                                        ),  // 8e /r       mov     sreg, r/m?
  ...
  CRegLabels : array [1..4, 0..7] of pchar = (( 'al',  'cl',  'dl',  'bl',  'ah',  'ch',  'dh',  'bh'),
                                              ( 'ax',  'cx',  'dx',  'bx',  'sp',  'bp',  'si',  'di'),
                                              ( 'es',  'cs',  'ss',  'ds',  'fs',  'gs',   nil,   nil),
...
type
  // reg state
  TRegState = array [0..7] of pointer;
...
function ParseCode(code         : pointer;
                   ...
                   var imVal    : integer;  // immediate value (in integer form, if available)
  ...
  begin
    case flgs and fI of
      ...
      fI32 : result := dw;  // dword/word immediate data (operand size prefix)
      else   begin
               result := 0;
               case opcode of
                 $9a, $ea : result := dw + 2;             // call/jmp $wwww:$dddddddd
                 $c8      : result := 3;                  // enter iw, ib
                 $a0..$a3 : if prfx67 then result := 2    // mov (e)ax/al <-> [$dddddddd]
                            else           result := 4;
               end;
             end;
    end;
  ...
  begin
    if flgs and fJmpRel <> 0 then begin
      ...
      result.PTarget     := code;
      result.Target      := pointer(integer(code) + integer(imLen) + imVal);
...
var prfxSeg   : byte;     // 26, 2e, 36, 3e, 64..65 segment override prefix
    ...
    imVal     : integer;  // immediate value (in integer form, if available)
  ...
  begin
    if (opcode < $d8) or (opcode > $df) then begin
      ...
      if prfx66 and ((flgs and f66 = f66R) or (flgs and f66 = f66RM)) then
        if (flgs and fMod <> 0) and ((flgs and fReg > fReg32) or (flgs and fMod > fMod32)) then
          // 66 prefix set for sse2 instructions means -> oword (16 bytes)
          len := 16
        else
          ...
          len := 2;
      case len of
        1, 2, 4 : result := CRegLabels[len, reg];
        8       : result := 'mm' + IntToStrEx(reg);
        16      : result := 'xmm' + IntToStrEx(reg);
      end;
    end else
  ...
  begin
    ...
    if (opcode <> $0f0f) and
      ...
      if ic2 = 0 then begin
        ...
        if imLen = 3 then
             ims := IntToHexEx(ic ) + ', ' + IntToHexEx(ic2)
    ...
    if FindModule(target, mh, mn) then begin
      ...
      if s1 = '' then begin
        ...
        if (nh <> nil) and (dword(target) >= mh) and
           (dword(target) <= mh + nh^.OptionalHeader.SizeOfImage) then begin
      ...
      if (s1 <> '') and (PosStr('(', s1) = 0) then begin
        // the target name was found, let's add the name of the module
        if mh <> HInstance then begin
  ...
  begin
    if FindModule(code, mh, s1) then begin
      nh := GetImageNtHeaders(mh);
      if nh <> nil then
        if (data >= mh) and (data <= mh + nh^.OptionalHeader.SizeOfImage - 100) then begin
          if isPtr and (TPCardinal(data)^ > mh) and
             (TPCardinal(data)^ <= mh + nh^.OptionalHeader.SizeOfImage - 100) then
...
begin
  ...
  result := ParseCode(code,
                      prfxSeg, prfx66, prfx67, prfxF0, prfxF2, prfxF3,
    ...
    if result.IsValid then begin
      ...
      if flgs and fMod <> 0 then begin
        ...
        case flgs and FReg of
          fReg8   : rs := RegStr(1, multi);                      // byte register
        ...
        case flgs and FReg of
          ...
          fRegO32  : rs := RegStr(4, opcode and $7);             // (d)word register depending on opcode
  ...
  with result do
    if (VirtualQuery(func, mbi, sizeOf(mbi)) = sizeOf(mbi)) and (mbi.State = MEM_COMMIT) then begin
      ...
      if ih <> nil then begin
        with ih^.OptionalHeader do begin
          dword(sh) := dword(ih) + sizeOf(ih^);
          if sh^.Characteristics and IMAGE_SCN_CNT_CODE <> 0 then begin
            ...
            mcb := dword(mbi.AllocationBase) + BaseOfCode;
            mce := mcb + SizeOfCode;
          end;
          mdb := dword(mbi.AllocationBase) + BaseOfData;
          mde := mdb + SizeOfUninitializedData + SizeOfInitializedData - 1;
...
begin
  ...
  if result.IsValid then begin
    if FindModule(func, mh, mn) then
         name := FindProcName(mh, func)
    else name := '';
    if name = '' then
...
begin
  if api = '' then begin
    ...
    if nh <> nil then
         result := @nh^.OptionalHeader.AddressOfEntryPoint
    else result := nil;
  end else
    result := GetImageProcAddress(ntdllhandle, DecryptStr(api));
end;
