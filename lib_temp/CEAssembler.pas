...
{ok}  (mnemonic:'ADC';opcode1:eo_reg2;opcode2:eo_ib;paramtype1:par_rm8;paramtype2:par_imm8;bytes:1;bt1:$80),//verified
  ...
  (mnemonic:'FSUBPR';opcode1:eo_pi;paramtype1:par_st;bytes:2;bt1:$de;bt2:$e0),
...
function GetCodeAssemble (Address: DWORD; OpCode: PChar; var CodeLen: Integer): Pointer; Stdcall;
var          
  RetCode: Array[Byte] of Byte;
  bytes: TAssemblerBytes;
begin
  Result := Nil;
  if Assemble (StrPas(OpCode), Address, bytes) then
  begin
    CodeLen := Length(bytes);
    CopyMemory (@RetCode[0], @bytes[0], CodeLen);
    RetCode[CodeLen] := $90;
    Result := @RetCode[0];
  end;
end;

function getAddressFromName(name:string):dword;
begin
  Raise Exception.Create('Unknow : ' + name);
end;
...
//  last:=-1;
  ...
  begin
    ...
    token:='['+inttohex(getaddressfromname(copy(token,2,length(token)-2)),8)+']';
  end;
...
//          disp:=0;
          ...
          try
            tokens[i]:=inttohex(getaddressfromname(tokens[i]),8);
